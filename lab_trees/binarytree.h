...
class BinaryTree
{
  ...
  private:
     ...
     *  as examples.
     */
    void mirror(Node* subRoot); //did not pass by reference, probably ok with here...
    
    bool isOrdered(const Node * subRoot) const; // helper function...

    void printPaths(const Node * subRoot,vector<T> v) const; // helprt function for printPath

    void printVector(vector<T> pathVec) const;
