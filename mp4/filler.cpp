                                 ...
                                 int tolerance, int frameFreq)
{
    ...
    solidColorPicker::solidColorPicker solid(fillColor);
                                ...
                                int gridSpacing, int tolerance, int frameFreq)
{
     ...
     */
    //return animation();
    gridColorPicker::gridColorPicker grid(gridColor, gridSpacing);
                                    ...
                                    int radius, int tolerance, int frameFreq)
{
     ...
     */
    //return animation();
     gradientColorPicker::gradientColorPicker gradient(fadeColor1, fadeColor2, radius, x, y);
                       ...
                       int tolerance, int frameFreq)
{
    ...
    OrderingStructure<int> X_order;
    OrderingStructure<int> Y_order;
    int frameNum = 0;
    ...
    // start point x,y
    RGBAPixel * originColor = img(x,y);
    int OrigRed = originColor->red;
    int OrigBlue = originColor->blue;
    int OrigGreen = originColor->green;

    X_order.add(x);
    Y_order.add(y);
    while(X_order.isEmpty() != true && Y_order.isEmpty() != true){
        int img_x = X_order.remove();
        int img_y = Y_order.remove();
        RGBAPixel * currColor = img(img_x,img_y);
        int currRed = currColor->red;
        int currBlue = currColor->blue;
        int currGreen = currColor->green;
        int d = pow((currRed - OrigRed), 2) + pow((currBlue - OrigBlue), 2) + pow((currGreen - OrigGreen), 2);

        if (processed[img_x][img_y] == false && d <= tolerance){
            
            processed[img_x][img_y] = true;
            *currColor = fillColor(img_x, img_y);
            frameNum ++;
