...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    (new_right->elements).assign(mid_elem_itr+1, child->elements.end()); // end - 1
    (new_right->children).assign(mid_child_itr, child->children.end()); // end -1
    (new_left->elements).assign(child->elements.begin(), mid_elem_itr); //mid_elem_itr -1
    (new_left->children).assign(child->children.begin(), mid_child_itr); //mid_elem_itr -1
