...
LogfileParser::LogfileParser(const string& fname) : whenVisitedTable(256)
{
    ...
    while (infile.good()) {
        ...
        string uniquekey = ll.url + ll.customer;
        ...
        if (whenVisitedTable.keyExists(uniquekey)){
            if(ll.date > whenVisitedTable[uniquekey])
                whenVisitedTable[uniquekey] = ll.date;
        }
        else
            whenVisitedTable.insert(uniquekey, ll.date);
