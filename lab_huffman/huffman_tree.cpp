                            ...
                            queue<TreeNode*>& mergeQueue)
{
    ...
    if (singleQueue.empty()){
        ...
        if (mergeQueue.empty()){
            ...
            int count1 = (node1->freq).getFrequency();
            int count2 = (node2->freq).getFrequency();
            if (count1 < count2){
                singleQueue.pop();
                return node1;
            }
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
    //using Iter = std::vector<Frequency>::const_iterator;
    ...
    while((singleQueue.size()+mergeQueue.size()) > 1){
        ...
        int count = (small1->freq).getFrequency() + (small2->freq).getFrequency();

        TreeNode * inter = new TreeNode(count);
        inter->left = small1;
        inter->right = small2;
        mergeQueue.push(inter);
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    while(bfile.hasBits()){
        if (bfile.getNextBit() == 1){
            char myChar = bfile.getNextByte();
            TreeNode * node = new TreeNode(Frequency(myChar, 0));
            return node;
        }
        else{
